...
Both SSH and verified OpenWrt-local adapters implement the same fixed recipe. Windows/Linux common paths have host-fixture evidence. The [isolated OpenWrt acceptance](emulator-validation-v7.md) is Linux-host SSH evidence, not Windows-to-router, on-device binary or BPI-R4 hardware acceptance. opkg and package installation/removal/upgrade remain unimplemented.
