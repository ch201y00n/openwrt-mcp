...
Current contract: **v22**, [ADR 0022](adr/0022-authenticated-ciphertext-foundations.md).
P3's [ciphertext foundations](ciphertext-foundations.md) add private memory staging,
authenticated pre-provisioned record storage, separate HMAC provenance, bounded
same-session SSH capture and full-authentication inspection. Record publication is
not native rename/ACL privacy. Exact new namespace/dependency edges are controlled
by the harness; twelve owners and all P2 authority boundaries remain. Validate and
commit this architecture checkpoint before production behavior.
