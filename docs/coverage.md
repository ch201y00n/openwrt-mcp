...
Twenty built-in reads are implemented: ten v6 typed response contracts, nine legacy scalar projections and one closed v7 package observation workflow. Their contracts are in [collection-read-contracts.md](collection-read-contracts.md), [passive wireless contracts](wireless-observation-contracts.md), [read-contracts.md](read-contracts.md), [storage observations](storage-observations.md) and [package observations](package-observations.md). The v6 shared 256-item budget is unchanged; v7 privately validates a bounded capture before returning 16-record pages. All normalized results retain the 64-KiB cap and whole MCP results the 256-KiB cap. No silent truncation or synthesized healthy/stopped state follows from missing or malformed data.
