...
- ADR 0008 splits protected-read availability from new `private_log_supported()`; audit configuration checks the latter. Windows protected reads cannot imply support for private append/rotation. Its first reviewed optional read profile requires absolute DOS paths and local trusted NTFS; relative paths remain a Linux profile feature, not a portable guarantee.
