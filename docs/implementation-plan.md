...
| Capability implementation | Local/SSH probes on the same selected target, strict parsers, offline CLI preservation, existing tool gating, operator-visible safe profile/support status | Planned |
...
| Isolated OpenWrt acceptance | Official 25.12.5 ARM64 QEMU userspace, then an opkg-family release and variant/negative fixtures | Planning; no live target writes |
