...
| v9 LuCI/storage observations | Two additional exact object descriptions; scoped mount/block/swap metadata in unchanged typed response owners | Architecture-only acf6532 passed both host gates before behavior. Two storage reads bring the catalog to 20; synthetic acceptance, no hardware/emulated storage claim. DHCP unions/composite identities still require architecture evolution |
...
| Full host facilities | Native Windows/macOS audit destinations, macOS protected files, optional actual Vault integration, native OS acceptance | Windows protected reads implemented under v8; remaining facilities stay explicitly unsupported |
