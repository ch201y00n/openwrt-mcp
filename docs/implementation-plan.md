...
| Stage | Scope | State |
| --- | --- | --- |
| Foundation v2-v4 | Policy/dispatcher/MCP, ten conservative reads, age/key sources, explicit local/SSH targets, host platform separation, architecture harness | Implemented; 192 Linux-on-WSL tests; native-host and device acceptance incomplete |
...
| Capability implementation | Local/SSH probes on the same selected target, strict parsers, offline CLI preservation, existing tool gating, operator-visible safe support status | Implemented; full gate passes with 260 distinct Linux-on-WSL tests; native-host acceptance remains |
| v6 bounded read architecture | Finite typed projections, bound local selectors, duplicate-rejecting response parser, global row/byte limits and five mandatory native suites | Architecture-only full gate passed with 272 tests (51 harness, five new declaration scaffolds); functional implementation has not started |
| Typed read coverage | Bounded record collections for network, wireless, services, packages, storage, DNS/DHCP, VPN, firewall and system/diagnostics | Planned; individual contracts/evidence required |
| Isolated OpenWrt acceptance | Official 25.12.5 ARM64 QEMU userspace, then an opkg-family release and variant/negative fixtures | First actual MCP/SSH run passed seven reads and three explicit unavailable cases; see emulator-validation.md. Broader coverage and opkg-family acceptance remain |
