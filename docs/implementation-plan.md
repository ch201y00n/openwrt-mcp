...
계획 버전: 2. 갱신: 2026-09-14 (Asia/Seoul). 최초 기준 커밋: `a711593`; P1 산출물 반영.
...
현재 아키텍처는 v20이고, 네 가지 추가 UCI 조회를 P1에서 구현했다.
...
다음 구현 진입점은 **P2**다. D1의 정확한 변경 프로필과 D2의 장치 접수/동시 변경 정책을
먼저 확정하고, D3 복구 권한·내구성을 포함한 ADR/spec/부정 하네스를 기능 코드보다 먼저 검증·커밋한다.
