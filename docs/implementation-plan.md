...
계획 버전: 3. 갱신: 2026-09-15 (Asia/Seoul). 최초 기준 커밋: `a711593`; P1/P2 산출물 반영.
...
현재 아키텍처는 v21이며, P2는 변경 경계·포트·상태·복구·하네스를 명세한다.
네 가지 추가 UCI 조회는 P1에서 구현했다. P2에서 변경 도구를 활성화하지 않는다.
...
이 체크포인트 검증·커밋 후 다음 구현 진입점은 **P3**다. 비밀/암호문 capture/store와
완전 인증된 제한 복원 어댑터부터 합성 자료로 구현한다. 추가 네이티브 ABI/의존성 변경이
필요하면 해당 부분의 ADR/하네스를 먼저 확장한다. P4 전에는 변경 도구를 완성했다고 하지 않는다.
