...
V11 adds exactly `uci`, making ten reviewed objects. Its description authorizes no method: core admits only six closed config/type/get recipes and rejects all custom UCI definitions. See [UCI source semantics and limits](uci-observations.md).

Observations last at most 30 seconds, measured by the host's monotonic clock. A private cache has at most ten entries, bounded by the closed object enum. Connection/authentication epoch changes or invalidation prevent reuse. Probe, cache waiting and command execution share one device-work deadline; byte limits cap each response and introspection has an additional 64-KiB ceiling. The start audit must succeed before probe/key/connection I/O. There is no automatic operation replay.
