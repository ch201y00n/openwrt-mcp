...
Subsequent [v10 MCP/SSH emulator acceptance](emulator-validation-v10.md) separately validates v2 with three mount rows and an empty block-device map. This extends the historical host-only scope above; it does not validate physical disks, nonempty block/swap entries or BPI-R4 hardware.
