...
Architecture v8 adds a narrowly scoped Windows SDK boundary and a mandatory Windows-only protected-read suite (not a portable zero-test success claim). The architecture checkpoint contains declarations only. The source gate rejects unsafe expressions and SDK access outside its exact owned file, all manual FFI/unsafe items, widened lint allowances and dependency-target changes. Safe policy code must not acquire native APIs. Synthetic Windows fixtures belong in a private, non-synced directory with a trusted ancestor chain; do not modify existing directory ACLs to make tests pass. Validate this checkpoint on native Windows and Linux before implementation.
