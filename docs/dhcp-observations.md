...
The subsequent [v10 emulator run](emulator-validation-v10.md) separately validates both tools with matching LuCI metadata and empty lease arrays. Populated lease rows, expiry sentinels and IPv6 prefixes remain synthetic host evidence, not inferred device acceptance.
