...
fn operation() -> Operation {
    serde_json::from_value(
        ...
        "action":{"kind":"apk_installed_page"},"capability":{"kind":"apk_installed_query"}}),
    )
    .unwrap()
}
fn setup(
...
async fn every_page_is_audited_but_capture_and_entropy_happen_only_once() {
    let (d, target, tokens, log) = setup(true, 1, 10000);
...
async fn denied_invalid_and_start_audit_failure_do_zero_capture_or_entropy() {
    let (denied, t, entropy, _) = setup(false, 1, 10000);
    assert_eq!(
    ...
    let (d, t, entropy, log) = setup(true, 1, 10000);
    for args in [
        json!({"program":"/bin/sh"}),
        json!({"cursor":""}),
        json!({"cursor":null}),
...
async fn metadata_remains_unknown_without_capturing_or_inventing_a_signature() {
    let (d, target, tokens, _) = setup(true, 1, 10000);
    let status = d
    ...
    assert_eq!(status.scope, "closed_query_response");
...
async fn replay_is_identical_and_foreign_or_invalid_offsets_never_recapture() {
    let (d, t, _, _) = setup(true, 1, 10000);
    ...
    let (other, other_target, _, _) = setup(true, 50, 10000);
...
async fn failed_refresh_and_entropy_failure_cannot_restore_the_previous_snapshot() {
    let (d, t, entropy, _) = setup(true, 1, 10000);
    let first = invoke(&d, json!({})).await.unwrap();
...
async fn timeout_and_completion_audit_failure_never_return_success() {
    let (d, t, _, _) = setup(true, 1, 100);
    t.stall.store(true, Ordering::SeqCst);
    assert_eq!(invoke(&d, json!({})).await.unwrap_err().code(), "timeout");
    let (d, t, _, log) = setup(true, 1, 10000);
...
async fn absent_or_zero_entropy_cannot_start_capture() {
    ...
    let (_, target, _, log) = setup(true, 1, 10000);
    let policy = Policy {
        categories: [(
            Category::Packages,
            Grant {
...
async fn late_non_yielding_entropy_is_rejected_before_backend_capture() {
    ...
    let (_, target, _, log) = setup(true, 1, 100);
    let policy = Policy {
        categories: [(
            Category::Packages,
            Grant {
