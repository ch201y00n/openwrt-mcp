...
v6 전체 검사에서 Linux-on-WSL 테스트 347개가 통과했습니다. 공식 OpenWrt 25.12.5 ARM64 에뮬레이터의 실제 MCP·SSH 호출에서는 새 형식의 조회 5개를 포함해 12개가 성공했고, 사용할 수 없는 2개는 명시적인 오류로 처리됐습니다. [v6 에뮬레이터 검증 범위](docs/emulator-validation-v6.md)와 [전체 검증·성능 기록](docs/validation.md)을 확인하세요. BPI-R4 하드웨어와 Windows·macOS 네이티브 검증은 아직 남아 있습니다.
