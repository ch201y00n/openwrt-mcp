...
v0.1 개발 단계입니다. 현재 시스템·네트워크·무선·서비스·진단에 걸친 상태 조회 14개를 구현했습니다. 이 중 5개는 형식과 크기를 명시한 목록·레코드 응답을 사용합니다. 실제 MCP 표준입출력 통신, 기본 차단 권한 엔진, 운영자 정의 확장 작업, 감사 로그, 리소스 제한과 자동 테스트를 제공합니다.

전체 검사에서 Linux-on-WSL 테스트 347개와 [Windows GNU 네이티브 테스트 315개](docs/windows-validation.md)가 통과했습니다. 공식 OpenWrt 25.12.5 ARM64 에뮬레이터의 실제 MCP·SSH 호출에서는 새 형식의 조회 5개를 포함해 12개가 성공했고, 사용할 수 없는 2개는 명시적인 오류로 처리됐습니다. [v6 에뮬레이터 검증 범위](docs/emulator-validation-v6.md)와 [전체 검증·성능 기록](docs/validation.md)을 확인하세요. BPI-R4 하드웨어, MSVC·macOS 검증과 Windows·macOS의 보호 파일 기능은 아직 남아 있습니다.
