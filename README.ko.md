...
v0.1 개발 단계입니다. 아키텍처 v2의 구조 검증을 마친 뒤 시스템·네트워크·무선·서비스·진단에 걸친 열 가지 상태 조회로 확장했습니다. 실제 MCP 표준입출력 통신, 기본 차단 권한 엔진, 운영자 정의 확장 작업, 감사 로그, 리소스 제한과 자동 테스트를 제공합니다. 현재 기능 검증은 가짜 장치를 사용하는 테스트 기준이며 실장비 검증과 구분합니다.
