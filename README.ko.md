...
v0.1 개발 단계입니다. 현재 시스템·네트워크·무선·서비스·진단에 걸친 상태 조회 17개를 구현했습니다. 이 중 8개는 형식과 크기를 명시한 목록·레코드 응답을 사용합니다. 실제 MCP 표준입출력 통신, 기본 차단 권한 엔진, 운영자 정의 확장 작업, 감사 로그, 리소스 제한과 자동 테스트를 제공합니다.

전체 검사에서 Linux-on-WSL 테스트 357개와 [Windows GNU 네이티브 테스트 325개](docs/windows-validation.md)가 통과했습니다. 앞선 공식 OpenWrt 25.12.5 ARM64 에뮬레이터 검증에서는 첫 형식화 조회 5개를 포함해 12개가 성공했고, 사용할 수 없는 2개는 명시적인 오류로 처리됐습니다. 이후 추가한 무선 단말·국가 조회는 아직 모의 데이터 기반 검증이며, 이 에뮬레이터 결과에 포함되지 않습니다. [v6 에뮬레이터 검증 범위](docs/emulator-validation-v6.md)와 [전체 검증·성능 기록](docs/validation.md)을 확인하세요. BPI-R4 하드웨어, MSVC·macOS 검증과 Windows·macOS의 보호 파일 기능은 아직 남아 있습니다.
...
무선 읽기 권한은 연결 단말 MAC 주소와 링크 통계도 공개합니다. 단말 식별자를 숨기려면 `wireless_stations`, `wireless_station_status`를 차단하세요. 새 조회 3개는 [기존 v6 구조 안에서 구현](docs/wireless-observation-contracts.md)했으며, 스캔·연결 해제·국가 설정은 하지 않습니다. 빈 목록은 드라이버 실패와 구분되지 않을 수 있으므로 단말 부재나 정상 상태의 증거가 아닙니다. 대규모 패키지 목록은 [별도 설계 초안](docs/package-inventory-design.md) 단계이며, 페이지 처리나 관리 기능이 구현됐다는 뜻은 아닙니다.
